...
import { ArrowDownRight, ArrowUpRight, X, Tag, Package, Calendar, Clock, CheckCircle2, Plus, ArrowLeft } from 'lucide-react';
...
export const MaterialMovementModal: React.FC<MaterialMovementModalProps> = ({ type, onClose, isInline }) => {
  ...
  const handleConfirmMaterial = (e: React.FormEvent) => {
    ...
    items.forEach(item => {
      ...
      const reasonText = finalNotes || (type === 'ENTRADA' ? 'Sem observação' : 'Sem observação');

      adjustStock(item.selectedMatId, changeNum, reasonText, currentUser?.name || 'Operador da Loja');
      successCount++;
    });
  ...
  const modalContent = (
    <div className={`${isInline ? 'w-full' : 'bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200'} space-y-4`}>
        ...
        <form onSubmit={handleConfirmMaterial} className="space-y-4">
          <div className="space-y-4 max-h-[50vh] overflow-y-auto overflow-x-hidden pr-1 pb-1 custom-scrollbar">
