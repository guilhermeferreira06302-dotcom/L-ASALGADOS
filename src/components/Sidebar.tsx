...
interface SidebarProps {
  ...
  employeeTab: 'SHIFT' | 'OPERACAO';
  setEmployeeTab: (tab: 'SHIFT' | 'OPERACAO') => void;
  openAIModal?: () => void;
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Material Inflow/Outflow modal state for Admin
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {activePortal === 'ADMIN' ? (
            <div className="space-y-1.5">
              ...
              >
                <div className="flex items-center gap-3">
                  <Clock className={`w-4 h-4 ${adminTab === 'SHIFT' ? 'text-emerald-400' : 'text-slate-700'}`} />
      ...
      {/* Material Inflow / Outflow Modal (Entrada/Saída de Material) from Sidebar (Admin only now) */}
      {materialModalType && (
        <MaterialMovementModal 
          type={materialModalType} 
          onClose={() => setMaterialModalType(null)} 
        />
      )}
    </>
  );
